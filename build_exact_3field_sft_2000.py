"""
Master Part 1 SFT Dataset Builder - Exact 3-Field Format
...
# Generate Authentic QA Response
def generate_academic_qa(board: str, grade: int, subject: str, topic: str, is_teacher: bool = False, locale_fact: Optional[str] = None) -> tuple:
    target_role = "TEACHER" if is_teacher else "STUDENT"
    
    # Generate natural user prompt variations without text dumps
...
# Generate Authentic Lesson Plan Response
def generate_academic_lp(board: str, grade: int, subject: str, topic: str, locale_fact: Optional[str] = None) -> dict:
...
# Generate Authentic Quiz Response
def generate_academic_quiz(board: str, grade: int, subject: str, topic: str, locale_fact: Optional[str] = None) -> dict:
    ...
    # Topic-specific real questions based on grade and subject
    if "Mathematics" in subject or "গণিত" in subject:
...
def build_complete_2000_dataset():
    print("Initializing Part 1 SFT Builder (Exact 3-Field Schema)...")
    ...
    # Verified locale snippets from locale.json for localized records
    ...
    for stage_name, cfg in targets.items():
        ...
        while qa_done < stage_qa_target:
            ...
            # Add locale fact if topic is geography or environmental science (approx 10-15% of records)
            loc_fact = None
            ...
            rec = generate_academic_qa(t["board"], t["grade"], t["subject"], t["topic"], is_teacher=is_teacher, locale_fact=loc_fact)
            
            p_key = (t["grade"], t["subject"], rec["user"].strip().lower(), qa_done)
        ...
        while lp_done < stage_lp_target:
            ...
            rec = generate_academic_lp(t["board"], t["grade"], t["subject"], t["topic"], locale_fact=loc_fact)
            p_key = (t["grade"], t["subject"], rec["user"].strip().lower(), lp_done)
            if p_key not in seen_prompts:
                seen_prompts.add(p_key)
                all_records.append(rec)
                lp_done += 1
        ...
        while quiz_done < stage_quiz_target:
            ...
            rec = generate_academic_quiz(t["board"], t["grade"], t["subject"], t["topic"], locale_fact=loc_fact)
            p_key = (t["grade"], t["subject"], rec["user"].strip().lower(), quiz_done)
            if p_key not in seen_prompts:
                seen_prompts.add(p_key)
                all_records.append(rec)
                quiz_done += 1
    ...
    # Strict Validation of exact 3 fields
    for idx, r in enumerate(all_records):
        keys = list(r.keys())
        assert set(keys) == {"system", "user", "assistant"}, f"Row {idx} has invalid keys: {keys}"
        assert len(r["system"]) > 50, f"Row {idx} system prompt too short"
        assert len(r["user"]) > 5, f"Row {idx} user prompt too short"
        assert len(r["assistant"]) > 50, f"Row {idx} assistant response too short"
        # Check no leaked chunk language
        assert "retrieved textbook chunk" not in r["assistant"].lower(), f"Row {idx} contains leaked chunk text"
        assert "প্রদত্ত পাঠ্যাংশ অনুযায়ী" not in r["assistant"], f"Row {idx} contains leaked text"
    ...
    for p in out_paths:
        ...
        print(f"Successfully saved {len(all_records)} exact 3-field records to: {p}")

if __name__ == "__main__":
    build_complete_2000_dataset()
