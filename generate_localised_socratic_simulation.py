"""
generate_localised_socratic_simulation.py
Runs the actual Socratic SimulationEngine across all 7 school boards, 10 STEM topics,
and 5 student personalities using a pre-configured LLM client mapping.
Generates 5,000 Socratic dialogues.
"""
...
from synthetictutor.core.schemas import (
    Concept, Misconception, StudentPersona, TeacherPersona, DialogueSession, DialogueTurn, DialogueRole, TeachingPlan
)
...
OUTPUT_PATH = "output/localised_socratic_simulation.jsonl"
TARGET_DIALOGUES = 5000

# 1. Custom LLM client that returns pre-configured Socratic turns to simulate the actual agents
class PredefinedLLMClient(BaseLLMClient):
    def __init__(self):
        self.current_student_u1 = ""
        self.current_teacher_u1 = ""
        self.current_student_u2 = ""
        self.current_teacher_u2 = ""
    ...
    async def generate_structured(
        ...
        if schema_name == "StudentResponseSchema":
            # If "STUDENT" is not in prompt, it's Turn 2
            if "STUDENT" not in prompt:
                return response_schema(
                    inner_thought="Expressing my localized background and traits.",
                    understanding_state="misconception_active",
                    response_text=self.current_student_u1
                )
            else:
                # Turn 4
                return response_schema(
                    inner_thought="Having a breakthrough based on teacher's scaffold.",
                    understanding_state="mastered",
                    response_text=self.current_student_u2
                )
                
        elif schema_name == "TeacherResponseSchema":
            # If prompt has exactly 1 "STUDENT" occurrence, it's Turn 3
            if prompt.count("STUDENT") == 1:
                return response_schema(
                    inner_thought="Scaffolding with a localized context analogy.",
                    pedagogical_intent="scaffold_hint",
                    response_text=self.current_teacher_u1
                )
            else:
                # Turn 5
                return response_schema(
                    inner_thought="Affirming mastery and concluding.",
                    pedagogical_intent="affirm",
                    response_text=self.current_teacher_u2
                )
                
        elif schema_name == "ConvergenceCheck":
            # If we've had at least 2 STUDENT inputs, mark as converged
            is_done = prompt.count("STUDENT") >= 2
            return response_schema(
                concept_mastered=is_done,
                dialogue_stalled=False,
                reason="Sufficient turns completed and concept mastered." if is_done else "Learning in progress."
            )
            
        return response_schema()

# 2. Regional Board Profiles & Cultural Markers
REGIONAL_PROFILES = {
    "cbse": {
...
# 4. Dialogue Patterns by Persona (incorporating regional/cultural keys)
PERSONA_PATTERNS = {
    "topper": {
        "name": "The Topper",
        "ability": "high",
...
# 5. Topic-Specific Dialogue Patterns mapped by [Topic] × [Persona]
TOPIC_PERSONA_TURNS = {
    "c_light": {
        "topper": {
    ...
    "c_micro": {
        "topper": {
            ...
            "t2": "Spendid! You have integrated microbial biochemistry with food science flawlessly."
        },
        "struggler": {
...
async def run_simulation():
    print("=========================================================")
    print("Executing Socratic Dialogue Simulation Generator...")
    print(f"Target Size: {TARGET_DIALOGUES} sessions")
    print("=========================================================")

    os.makedirs("output", exist_ok=True)
    
    # Instantiate custom preconfigured SFT LLM client
    llm = PredefinedLLMClient()
    ...
    with open(OUTPUT_PATH, "w", encoding="utf-8") as f_out:
        while sessions_written < TARGET_DIALOGUES:
            ...
            # Format text turns for predefined LLM
            s1 = turns_dict["s1"].format(**p_dict)
            t1 = turns_dict["t1"].format(**p_dict)
            s2 = turns_dict["s2"].format(**p_dict)
            t2 = turns_dict["t2"].format(**p_dict)
            
            # Regional translations override
            if lang == "hi" and board_key in ("upboard", "bihboard"):
                if topic_key == "c_acids":
                    s1 = f"नमस्ते टीचर। पीएच (pH) मान तो मैं समझती हूँ। पर हमारी रसोई में ताँबे के बरतनों को चमकाने के लिए इमली (acid) का उपयोग क्यों होता है?"
                    t1 = "बहुत बढ़िया! ताँबे के बर्तनों पर एक क्षारीय (basic) कॉपर कार्बोनेट की परत जम जाती है। इमली में टार्टरिक अम्ल (acid) होता है जो उसे उदासीन (neutralize) कर देता है। इससे क्या बनेगा?"
                    s2 = "अच्छा! तो अम्ल और क्षार मिलकर उदासीन अभिक्रिया करते हैं, और वह हरी परत घुलकर बह जाती है। बर्तन चमकने लगता है!"
                    t2 = "बिल्कुल सही! हमारे घरों में रोज़ाना ऐसी कई रासायनिक अभिक्रियाएँ होती हैं। शाबाश!"
                elif topic_key == "c_crops":
                    s1 = f"टीचर, हमारे यहाँ खेतों में {profile['crops'][0]} की फसल केवल सर्दियों (rabi) में ही क्यों बोई जाती है? मानसून में क्यों नहीं?"
                    t1 = f"बहुत अच्छा प्रश्न! {profile['crops'][0]} को बढ़ने के लिए कम तापमान और कम पानी चाहिए। मानसून में तापमान और वर्षा बहुत अधिक होती है। यदि हम इसे मानसून में बोएँगे तो क्या होगा?"
                    s2 = f"अधिक पानी और गर्मी से फसल के पौधे सड़ जाएँगे! इसलिए इसे सर्दियों के शुष्क मौसम में ही बोया जाता है। समझ गया!"
                    t2 = "बिल्कुल सही! यही कारण है कि फसलों को रबी और खरीफ ऋतुओं में उनकी जल-आवश्यकता के अनुसार बाँटा गया है।"
                else:
                    s1 = f"नमस्ते टीचर। मैं {profile['board_name']} से हूँ। मुझे {concept_name} का सिद्धांत समझ आता है पर हमारे यहाँ {local_object} में यह कैसे काम करता है?"
                    t1 = f"बहुत अच्छा प्रश्न! {local_object} में {concept_name} का सिद्धांत इस प्रकार काम करता है: {localization_desc}. इसे ऐसे समझो..."
                    s2 = f"अच्छा! तो इसका मतलब है कि {local_soil} और {local_object} दोनों भौतिक नियमों के अनुसार ही काम करते हैं। समझ गई!"
                    t2 = "बिल्कुल सही! हमारे आस-पास का वातावरण और खेती-बाड़ी सब विज्ञान के सिद्धांतों से ही जुड़ी हुई है।"
            elif lang == "bn" and board_key == "wbboard":
                if topic_key == "c_acids":
                    s1 = f"নমস্কার দিদিমণি। পরীক্ষার জন্য তো অ্যাসিড ও ক্ষারের সংজ্ঞা মুখস্থ করেছি। কিন্তু আমাদের পুজো ঘরে তামার প্রদীপ পরিষ্কার করতে তেঁতুল ব্যবহার করা হয় কেন?"
                    t1 = "দারুণ প্রশ্ন! তামার ওপর ক্ষারীয় কपर কার্বনেটের একটি সবুজ আস্তরণ পড়ে। তেঁতুলে থাকা টারটারিক অ্যাসিড সেটিকে প্রশমিত (neutralize) করে দেয়। এর ফলে কী ঘটে বলতো?"
                    s2 = "অ্যাসিড আর ক্ষার বিক্রিয়া করে লবণ ও জল তৈরি করে, ফলে ওই সবুজ আস্তরণটি ধুয়ে যায় এবং তামার প্রদীপটি আবার চকচকে হয়ে ওঠে!"
                    t2 = "একদম ঠিক! বিজ্ঞানের এই তত্ত্বগুলো আমাদের দৈনন্দিন জীবনে এভাবেই লুকিয়ে আছে। খুব ভালো!"
                else:
                    s1 = f"নমস্কার দিদিমণি/শিক্ষক মহাশয়। {concept_name} সম্পর্কে বইতে পড়েছি, কিন্তু আমাদের ওখানকার {local_object} এর সাথে এর কী সম্পর্ক?"
                    t1 = f"দারুণ প্রশ্ন! আসলে আমাদের ওখানকার {local_object} এর ক্ষেত্রে {concept_name} নীতিটি এইভাবে কাজ করে: {localization_desc}. বিষয়টি সহজভাবে বুঝিয়ে বলছি..."
                    s2 = f"বাহ! তাহলে তো বোঝা যাচ্ছে যে {local_soil} এবং {local_object} এর ওপর এই নীতিগুলোই খাটছে। এবার ধারণাটি পরিষ্কার হলো।"
                    t2 = "একদম ঠিক! নিজের চারপাশের পরিবেশ ও ঐতিহ্যের সাথে বিজ্ঞানকে মেলাতে পারলে পড়া আরও সহজ ও আনন্দদায়ক হয়।"
            elif lang == "ta" and board_key == "tnboard":
                if topic_key == "c_acids":
                    s1 = f"வணக்கம் ஐயா. அமிலங்கள் மற்றும் காரங்கள் நடுநிலையாக்கல் வினையை உருவாக்குகிறது என்று படித்தேன். ஆனால் நம் பூஜை அறையில் உள்ள காமாட்சி விளக்கை புளி போட்டு துலக்கினால் ஏன் பளபளக்கிறது?"
                    t1 = "சிறப்பான கேள்வி! காமாட்சி விளக்கின் மேல் காரத்தன்மை கொண்ட காப்பர் கார்பனேட் படிகிறது. புளியில் உள்ள டார்டாரிக் அமிலம் அதனை நடுநிலையாக்குகிறது (neutralize). இதனால் என்ன உருவாகிறது?"
                    s2 = "அமிலமும் காரமும் வினைபுரிந்து நீரில் கரையக்கூடிய உப்பை உருவாக்குகிறது, இதனால் அந்த பச்சை நிறப் படிவு நீங்கி விளக்கு பளபளக்கிறது!"
                    t2 = "மிகச் சரி! நம் வீட்டு பூஜை அறையில் கூட வேதியியல் நடுநிலையாக்கல் வினை எவ்வாறு செயல்படுகிறது என்பதைப் புரிந்து கொண்டாய். நன்று!"
                else:
                    s1 = f"வணக்கம் ஐயா/அம்மா. {concept_name} என்ற தலைப்பை புத்தகத்தில் படித்தேன், ஆனால் நம் ஊர் {local_object} உடன் இது எவ்வாறு தொடர்பு கொள்கிறது?"
                    t1 = f"சிறப்பான கேள்வி! நம் பகுதியில் உள்ள {local_object}-ல் {concept_name} கொள்கை எவ்வாறு தான் செயல்படுகிறது: {localization_desc}. இதைப் புரிந்து கொள்ள எளிய உதாரணம்..."
                    s2 = f"புரிகிறது! அப்படியானால் {local_soil} மற்றும் {local_object} இரண்டுமே அறிவியல் விதிகளுக்கு உட்பட்டவை தான். இப்போது தெளிவாகப் புரிகிறது."
                    t2 = "மிகச் சரி! நம் மண்ணையும் பண்பாட்டையும் அறிவியலோடு இணைத்துப் பார்க்கும்போது தான் கற்றல் முழுமையடைகிறது."
            elif lang == "mr" and board_key == "mahboard":
                s1 = f"नमस्कार सर. पुस्तकात {concept_name} ची व्याख्या वाचली, पण आपल्या भागातील {local_object} च्या संदर्भात हे कसे घडते?"
                t1 = f"खूपच छान प्रश्न! आपल्याकडील {local_object} च्या बाबतीत {concept_name} चा सिद्धांत या प्रकारे काम करतो: {localization_desc}. सोप्या भाषेत सांगायचे तर..."
                s2 = f"हो, आता समजले! म्हणजेच {local_soil} आणि {local_object} दोन्हीवरही हेच वैज्ञानिक नियम लागू होतात. माझी संकल्पना स्पष्ट झाली."
                t2 = "नक्कीच! आजूबाजूच्या शेत-परिसराला आणि संस्कृतीला विज्ञानाशी जोडल्यास शिक्षण अधिक सोपे आणि मजेशीर होते."
            elif lang == "pa" and board_key == "punboard":
                s1 = f"ਸਤਿ ਸ੍ਰੀ ਅਕਾਲ ਜੀ। ਮੈਨੂੰ {concept_name} ਦਾ ਸਿਧਾਂਤ ਸਮਝ ਆਉਂਦਾ ਹੈ, ਪਰ ਸਾਡੇ ਇਲਾਕੇ ਦੇ {local_object} ਨਾਲ ਇਹਦਾ ਕੀ ਸਬੰਧ ਹੈ?"
                t1 = f"ਬਹੁਤ ਵਧੀਆ ਸਵਾਲ! ਸਾਡੇ ਇਲਾਕੇ ਵਿੱਚ {local_object} ਉੱਤੇ {concept_name} ਦਾ ਨਿਯਮ ਇਸ ਤਰ੍ਹਾਂ ਲਾਗੂ ਹੁੰਦਾ ਹੈ: {localization_desc}. ਇਹਨੂੰ ਸੌਖੇ ਤਰੀਕੇ ਨਾਲ ਸਮਝਾਉਂਦਾ ਹਾਂ..."
                s2 = f"ਅੱਛਾ! ਹੁਣ ਸਮਝ ਆਇਆ ਕਿ ਸਾਡੀ {local_soil} ਅਤੇ {local_object} ਦੋਵੇਂ ਵਿਗਿਆਨਕ ਨਿਯਮਾਂ ਮੁਤਾਬਕ ਹੀ ਕੰਮ ਕਰਦੇ ਹਨ।"
                t2 = "ਬਿਲਕੁਲ ਠੀਕ! ਆਪਣੇ ਆਲੇ-ਦੁਆਲੇ ਅਤੇ ਸੱਭਿਆਚਾਰ ਨੂੰ ਵਿਗਿਆਨ ਨਾਲ ਜੋੜ ਕੇ ਦੇਖਣਾ ਹੀ ਅਸਲ ਪੜ੍ਹਾਈ ਹੈ।"
                
            # Set variables for custom LLM client
            llm.current_student_u1 = s1
            llm.current_teacher_u1 = t1
            llm.current_student_u2 = s2
            llm.current_teacher_u2 = t2
            
            # Setup concept schema
            concept = Concept(
                id=f"c_{topic_key}_{board_key}",
                name=concept_name,
                description=cbse_concept,
                domain="STEM",
                grade_level="Grade 9",
                prerequisite_ids=[],
                misconceptions=[]
            ...
            # Setup student/teacher personas
            student_persona = StudentPersona(
                id=f"student_{persona_key}_{board_key}",
                name=persona_info["name"],
                grade_level="Grade 9",
                prior_knowledge_score=0.5,
                active_misconceptions=[],
                curiosity_level=0.8,
                communication_style=persona_info["traits"]
            )
            
            teacher_persona = TeacherPersona(
                id="t_sarg_01",
                name="SARG Socratic Tutor",
                teaching_style="Socratic Scaffolding & Regional Adaptation",
                scaffolding_patience=0.9,
                tone="encouraging, curious, and localized"
            )
            
            # Run simulation using actual SimulationEngine implementation!
            session = await engine.run_simulation(plan, student_persona, teacher_persona, language=lang)
            
            # Export as SFT formatted ShareGPT style record
            dialogue_id = f"localised_sarg_sim_{sessions_written + 1:06d}"
            
            conv_list = []
            for turn in session.turns:
                role_val = "human" if turn.role == DialogueRole.STUDENT else "gpt"
                conv_list.append({"from": role_val, "value": turn.content})
            ...
            rec = {
                ...
                "metadata": {
                    ...
                    "student_persona": student_persona.name,
                    "student_style": student_persona.communication_style,
                    "teacher_style": teacher_persona.teaching_style,
            ...
            if sessions_written % 1000 == 0:
                print(f"Progress: {sessions_written:,} / {TARGET_DIALOGUES:,} simulated dialogues written...")
    ...
    print("=========================================================")
    print(f"SUCCESS! Simulated Socratic dataset generated at: {OUTPUT_PATH}")
